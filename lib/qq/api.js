...
export class QqApi {
  ...
  /**
   * List this bot's instruction panels ("查询指令面板列表").
   *
   * @param scope - the scene to filter by: `'c2c'` or `'group'`.
   * @returns The response body, whose `records` carry `panel_id`, `scope` and
   *   the panel itself.
   */
  async listPanels(scope) {
    // `scope` is required, not optional: the platform filters by scene and
    // answers 40030011 (生效场景不合法) when it is missing, which reads like a
    // bad scene rather than a missing parameter.
    if (typeof scope !== 'string' || scope === '') throw new QqApiError('QQ panel list needs a scope')
    return this.#request('GET', `/v2/panels?scope=${encodeURIComponent(scope)}`)
  }

  /**
   * Create one instruction panel ("创建指令面板").
   *
   * @param payload - `scope`, `target_type`, the target OpenIDs, and `panel`.
   * @returns The created record, including its `panel_id`.
   */
  async createPanel(payload) {
    return this.#request('POST', '/v2/panels', payload)
  }

  /**
   * Replace one panel's elements and remark ("修改指令面板").
   *
   * The associated users or groups are untouched, which is what makes a
   * reinstall idempotent.
   *
   * @param panelId - the panel to update.
   * @param panel - `{ items, remark }`.
   * @returns The response body.
   */
  async updatePanel(panelId, panel) {
    if (typeof panelId !== 'string' || panelId === '') throw new QqApiError('QQ panel update was given no panel id')
    return this.#request('PUT', `/v2/panels/${encodeURIComponent(panelId)}`, { panel })
  }
