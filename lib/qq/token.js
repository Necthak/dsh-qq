...
export class QqTokenProvider {
   ...
   */
  constructor({ appId, clientSecret, fetchImpl, now, log, timeoutMs = TOKEN_TIMEOUT_MS }) {
    // Injectable for the same reason the API client's is: a test that waits on
    // the real deadline either takes thirty seconds or, on a runner where the
    // unref'd timer is the only handle left, never gets there at all.
    this.timeoutMs = Number.isSafeInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : TOKEN_TIMEOUT_MS
    this.#appId = appId
