...
const WRITABLE_BOOLEAN = ['enabled', 'allowAllWhenEmpty', 'allowAgentSend', 'forwardApprovals', 'forwardQuestions', 'useMarkdown']
const WRITABLE_NUMBER = ['approvalTimeoutMs', 'questionTimeoutMs', 'maxBytes', 'intervalMs', 'progressIntervalMs', 'lowBalanceThreshold']
const WRITABLE_STRING = ['mode', 'ownerOpenId', 'workspacePath', 'agentPreset', 'busyDelivery', 'restartCommand']
...
function sanitizePatch(body) {
  ...
  for (const key of WRITABLE_NUMBER) {
    ...
    if (typeof value === 'number' && Number.isSafeInteger(value) && value > 0) patch[key] = value
  }
