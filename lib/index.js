...
import { currencySign, formatWindowAlert, lowBalances, readCredit, windowAlerts } from './bridge/credit.js'
...
export const SETTINGS_SCHEMA = z.object({
  ...
  /**
   * How many chunks an answer may occupy before it goes out as a file instead.
   * Zero disables that and sends every chunk. A measured deployment shows why
   * the limit exists: a long answer exhausts the passive reply window, then
   * spends the active-message quota, and when the operator has switched active
   * messages off the tail of the answer is lost outright.
   */
  longAnswerChunks: z.number().default(4),
  ...
  /**
   * Warn once a day when a subscription window has filled to this percentage.
   * Zero disables the warning. The windows, not the balance, are what stop a
   * subscription account, and they fill silently: the provider begins refusing
   * requests mid-task with no notice of its own.
   */
  planAlertPercent: z.number().default(80),
  /**
...
export function apply(ctx) {
  ...
  /**
   * Last plan-window warning. Separate from the balance's guard on purpose: the
   * two conditions are independent, and one warning must not spend the other's
   * day.
   */
  let planWarnedOn = ''
  ...
  /**
   * Warn once a day when a subscription window has nearly filled.
   *
   * Runs beside the balance check because both answer the same question — "is
   * the account about to stop working" — but keeps its own daily guard: the two
   * conditions are independent, and a healthy balance would otherwise silence
   * the warning that matters on a subscription plan. The message names the
   * window, its fill and its reset, and points at `/usage`, because a window
   * that fills again tomorrow needs a place to look that is not this one line.
   */
  function checkPlanWindows() {
    const threshold = Number.isFinite(current.planAlertPercent) ? current.planAlertPercent : 0
    if (threshold <= 0) return
    const alerts = windowAlerts(credit().snapshots, threshold)
    if (alerts.length === 0) return
    const today = new Date().toISOString().slice(0, 10)
    if (planWarnedOn === today) return
    planWarnedOn = today
    log(`plan window at or above ${String(threshold)}%: ${alerts.map((alert) => `${alert.name} ${alert.label} ${String(alert.percent)}%`).join(', ')}; warning the owner`)
    notifyAll(formatWindowAlert(alerts))
  }
   ...
   *
   * Every check is throttled here rather than given its own timer: this process
   * already wakes every fifteen seconds, and a second timer would be another
   * thing to dispose. The balance and the plan windows share the reading tick
   * but not the guard — see {@link checkPlanWindows} — and each is caught
   * separately so that one unreadable file cannot silence the other warning.
   */
  function housekeepingTick() {
    ...
    if (now - balanceCheckedAt >= BALANCE_CHECK_MS) {
      ...
      try {
        ...
        log(`plan window check failed: ${String(error?.message ?? error)}`)
      }
    }
  ...
  /**
   * Read one session projection, if this deployment can supply it.
   *
   * Both `/todos` and `/status` read DSH's own projections through this: resolve
   * the session's agent, then ask the projection registry for the state
   * belonging to it. Absence at any step is a deployment fact rather than a
   * failure — a profile need not mount the plugin whose projection is being read
   * — so it is reported as null and the caller then says nothing.
   */
  const projectionOf = createProjectionReader({ ctx, log })
  ...
  function todos(sessionId) {
    const state = projectionOf(sessionId, 'todos')
    return state === null ? null : state.then((value) => (Array.isArray(value) ? value : null))
  }

  /**
   * Read the session's active scheduled reminders for `/status`.
   *
   * The records belong to DSH's schedule plugin, which folds them into the
   * `schedule` projection; the state carries `{ active, seenIds }` and only
   * `active` is of interest here. A deployment without that plugin has no such
   * projection at all, which `/status` reports by saying nothing rather than by
   * presenting an error about a feature nobody asked for.
   *
   * @param sessionId - the DSH session behind the conversation.
   * @returns The active records, or null when the projection is unavailable.
   */
  function reminders(sessionId) {
    const state = projectionOf(sessionId, 'schedule')
    return state === null ? null : state.then(activeReminders)
  }
...
/**
 * Build the "read one session projection" accessor.
 *
 * Kept at module level, like the busy probe, so a deployment shape that cannot
 * answer — no agent resolver, no projection registry, a resolver that throws —
 * is provable in a test instead of being discovered on a phone.
 *
 * @param options - wiring.
 * @param options.ctx - the plugin context.
 * @param options.log - diagnostics sink.
 * @returns `(sessionId, key) => Promise<state> | null`.
 */
export function createProjectionReader({ ctx, log }) {
  return (sessionId, key) => {
    try {
      const resolved = ctx.sessionController?.resolveAgent?.(sessionId)
      // A resolver that answers synchronously is not one this can wait on, and
      // guessing an answer is worse than admitting the projection is unreadable.
      ...
      return resolved.then((settled) => {
        ...
        return projections?.stateOf?.(agent.session, key)
      })
    } catch (error) {
      log(`${key} projection could not be read (${String(error?.message ?? error)})`)
      return null
    }
  }
}
...
/**
 * Pull the active reminders out of the `schedule` projection's state.
 *
 * The schedule projection keeps `{ inheritedEventCount, active, seenIds }`. Only
 * `active` is of interest here, and a state that does not carry a usable array —
 * an older DSH, a projection that failed to initialize, or no projection at all,
 * which is what a profile without the schedule plugin reports — yields null so
 * `/status` says nothing rather than presenting an error about a feature the
 * deployment may never have had.
 *
 * @param state - the projection state, or whatever the registry returned.
 * @returns The active records, or null when there are none to read.
 */
export function activeReminders(state) {
  return Array.isArray(state?.active) ? state.active : null
}
