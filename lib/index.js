...
export function apply(ctx) {
  ...
  /**
   * Read one session's whole-log figures.
   *
   * `sessionStats` is a projection, not a service: it is registered beside the
   * projection registry and folded from the durable log, which is why the
   * numbers survive paging and compaction. A session with no completed step yet
   * legitimately has none.
   *
   * @param sessionId - the DSH session behind the conversation.
   * @returns The stats value, or null when it cannot be read.
   */
  async function usage(sessionId) {
    try {
      ...
      return projections?.stateOf?.(agent.session, 'sessionStats') ?? null
    } catch (error) {
      log(`usage could not be read (${String(error?.message ?? error)})`)
      return null
    }
  }
  ...
  const commands = createSessionCommands({ ctx, sessions, pending, settingsScope: scope, restart: restarts, busy, usage, screenshot, log })
...
function sessionTablePath() {
  ...
  if (typeof home === 'string' && home !== '') return `${home}/dsh-qq-sessions.json`
  const userProfile = process.env.USERPROFILE ?? process.env.HOME ?? '.'
  return `${userProfile}/.dsh/dsh-qq-sessions.json`
}
