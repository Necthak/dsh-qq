...
import { buildMenuKeyboard } from './keyboard.js'
...
/**
 * Words that stand in for a command, so a phone does not have to type a slash.
 *
 * Matched against the WHOLE message after trimming, which is what makes them
 * safe: 「任务」 is a command while 「任务完成了」 is a sentence, and no ordinary
 * message is swallowed by accident. They also make the commands reachable by
 * voice, because the platform transcribes a voice message into exactly this text.
 */
...
export function createInboundHandler({ ctx, sessions, outbound, pending, models, commands, config, log, ensureSession, fetchImpl, status, busy, progress, onRejected, signal }) {
  ...
  return async function handleMessage(message) {
    ...
    // answer.
    // A shortcut word is rewritten into the command it stands for, so every rule
    // downstream - admission, owner checks, output - stays exactly where it was.
    const aliased = COMMAND_ALIASES.get(text)
    const commandText = aliased === undefined ? text : `/${aliased}`

    if (override === null && commandText === '/menu') {
      // The menu is the one command that needs the outbound client and the
      // keyboard builder at hand, so it is answered here rather than in the
      // command module, which only ever sends text.
      try {
        await outbound.deliver({
          key,
          kind: message.kind,
          peerId: message.peerId,
          text: '📋 快捷菜单：点一下即可。也可以直接说「状态」「额度」「任务」…',
          keyboard: buildMenuKeyboard(),
        })
      } catch (error) {
        log(`${key}: menu could not be delivered: ${String(error?.message ?? error)}`)
      }
      return
    }
...
async function runCommand({ models, commands, sessions, outbound, config, log, message, key, text, status, busy, progress, signal }) {
  ...
  if (name === 'help') {
    await send([
      ...
      '/usage — 本会话用量：轮次、步骤、模型/工具耗时、输出 tokens',
      '/log [行数] — 查看最近的本插件日志（仅 owner，默认 15 条）',
      '/doctor — 主动自检：通道、看门狗、凭据、owner、余额、发送失败（仅 owner）',
      '/todos — 查看 agent 本轮的任务清单与进度',
      '/menu — 发一条带按钮的快捷菜单（状态/额度/任务/会话/诊断/截图）',
      '快捷词：不打斜杠也行 —— 直接说「状态」「额度」「任务」「会话」「诊断」「截图」「日志」「菜单」',
