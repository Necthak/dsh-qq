 ...
 * @param options.reminders - reads the session's active scheduled reminders for
 *   `/status`: `(sessionId) => Promise<records | null>`, where null means the
 *   deployment has no schedule projection at all.
 ...
 */
export function createInboundHandler({ ctx, sessions, outbound, pending, models, commands, config, log, ensureSession, fetchImpl, status, busy, reminders, progress, onRejected, signal }) {
  ...
  return async function handleMessage(message) {
    ...
    if (override === null && commandText.startsWith('/')) {
      const handled = await runCommand({ models, commands, sessions, outbound, config, log, message, key, text: commandText, status, busy, reminders, progress, signal })
      if (handled) return
    }
 ...
 */
async function runCommand({ models, commands, sessions, outbound, config, log, message, key, text, status, busy, reminders, progress, signal }) {
  ...
  if (name === 'help') {
    await send([
      ...
      '',
      '【对话】',
      '/status — 通道、会话、当前模型、回合状态、定时提醒、最近一次发送失败',
  ...
  if (name === 'status') {
    ...
    // Reminders belong to the session, so they are read only once one exists.
    // A deployment without DSH's schedule plugin has no `schedule` projection,
    // and `formatReminders` then adds no lines at all: the absence of a plugin
    // is not an error about this session, and reporting it as one would teach
    // the operator to ignore the line.
    if (record !== undefined && typeof reminders === 'function') {
      let list = null
      try {
        list = await reminders(record.sessionId)
      } catch (error) {
        log(`reminders could not be read: ${String(error?.message ?? error)}`)
      }
      lines.push(...formatReminders(list))
    }
...
/**
 * Render the session's scheduled reminders for `/status`.
 *
 * The records come from DSH's schedule plugin, so their shape is that plugin's
 * to change: every field is read as possibly absent, and one that cannot be
 * read is reported as unknown rather than guessed at, because a wrong firing
 * time is worse than an admitted gap. The plugin distinguishes a one-shot
 * (`after`, `at`) from a fixed rate (`every`), and the rate is worth naming:
 * "next at 10:00" alone does not tell the operator whether that is the only
 * reminder left.
 *
 * A null list means the projection itself is unavailable — the schedule plugin
 * is not mounted — and produces no lines: that is a fact about the deployment,
 * not a fault of the session.
 *
 * @param list - the projection's active records, or null when unreadable.
 * @param now - the instant to compare targets against.
 * @returns The lines to append to `/status`, possibly none.
 */
export function formatReminders(list, now = new Date()) {
  if (!Array.isArray(list)) return []
  if (list.length === 0) return ['定时提醒：无']
  const lines = [`定时提醒：${String(list.length)} 条`]
  for (const record of list) lines.push(`· ${describeReminder(record, now)}`)
  return lines
}

/**
 * Describe one reminder record in a single line.
 *
 * @param record - one record from the `schedule` projection.
 * @param now - the instant to compare the target against.
 * @returns The reminder's content and when it fires.
 */
function describeReminder(record, now) {
  const prompt = typeof record?.prompt === 'string' && record.prompt.trim() !== '' ? record.prompt.trim() : '（未记录内容）'
  // `everySeconds` is present only on a fixed-rate record, and a record whose
  // kind says `every` but carries no usable interval is still reported, just
  // without the rate: dropping the reminder entirely would hide it from the
  // only surface that lists it.
  const seconds = Number(record?.everySeconds)
  const rate = record?.kind === 'every' && Number.isFinite(seconds) && seconds > 0 ? `每 ${intervalLabel(seconds)} · ` : ''
  const at = typeof record?.scheduledAt === 'string' ? new Date(record.scheduledAt) : null
  if (at === null || Number.isNaN(at.getTime())) return `${rate}${prompt}（时间未知）`
  // An active record whose target has passed is waiting for the session to
  // become live; saying so explains a time in the past instead of leaving it
  // looking like a bug.
  const overdue = at.getTime() <= now.getTime() ? ' · 已到期' : ''
  return `${rate}${prompt}（${stamp(at)}${overdue}）`
}

/**
 * Render an instant as a short local stamp.
 *
...
/**
 * Render an interval the way a person would say it.
 *
 * @param seconds - the interval in seconds.
 * @returns A short form such as `5 分钟` or `2 小时`.
 */
function intervalLabel(seconds) {
  if (seconds % 3600 === 0) return `${String(seconds / 3600)} 小时`
  if (seconds >= 60) return `${String(Math.round(seconds / 60))} 分钟`
