...
export const BUTTON = {
  ...
  option: 'opt',
  /** `cmd|<name>` — run one bridge command, as if it had been typed. */
  command: 'cmd',
}
...
export function decodeButton(data) {
  ...
  if (kind === BUTTON.command) return parts.length === 2 ? { kind, args: parts.slice(1) } : null
  return null
}
...
export function buildMenuKeyboard() {
  // Six of the commands that are awkward to type on a phone, in the order they
  // are usually wanted: what is happening, what it costs, what it is working
  // on, which conversations exist, whether anything is wrong, and a screenshot.
  // Everything else stays available as a command; this is the short list.
  return keyboard([
    [
      button('状态', `${BUTTON.command}|status`),
      button('额度', `${BUTTON.command}|usage`),
      button('任务', `${BUTTON.command}|todos`),
    ],
    [
      button('会话', `${BUTTON.command}|sessions`),
      button('诊断', `${BUTTON.command}|doctor`),
      button('截图', `${BUTTON.command}|screen`, STYLE.blue),
    ],
  ])
}

/**
 * The commands the menu offers, for the help text and the tests.
 *
 * @returns The command names, in button order.
 */
export function menuCommands() {
  return ['status', 'usage', 'todos', 'sessions', 'doctor', 'screen']
}
