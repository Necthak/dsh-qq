...
export class Outbound {
  ...
  #read() {
    ...
    return {
      ...
      useMarkdown: raw.useMarkdown === true,
    }
  }
  ...
  sendActive({ key, kind, peerId, text, keyboard }) {
    ...
    const next = previous
      .catch(() => {})
      .then(() => this.#sendChunks({ key, kind, peerId, chunks: this.#prepare(text), usePassive: false, keyboard }))
    this.#chains.set(key, next)
    next.finally(() => {
      if (this.#chains.get(key) === next) this.#chains.delete(key)
    }).catch(() => {})
  ...
  sendQuoted({ key, kind, peerId, text, msgId, msgSeq }) {
    ...
    const next = previous
      .catch(() => {})
      .then(async () => {
        const chunks = this.#prepare(text)
        ...
        for (let index = 0; index < chunks.length; index += 1) {
          ...
          // Only the first chunk quotes; the rest continue the same answer.
          const body = index === 0 ? this.#body(chunks[index], { msgId, msgSeq }) : this.#body(chunks[index])
          try {
   ...
   */
  #prepare(text) {
    const { maxBytes, useMarkdown } = this.#read()
    const body = useMarkdown ? text : markdownToPlain(text)
    return splitByBytes(body, maxBytes)
  }
   ...
   */
  #body(chunk, reply, keyboard) {
    const { useMarkdown } = this.#read()
    ...
    // So a keyboard decides the content type here, and `useMarkdown` only
    // decides it for the messages that carry no buttons.
    const content = useMarkdown || keyboard !== undefined ? { markdown: chunk } : { text: chunk }
  ...
  async #deliverNow({ key, kind, peerId, text, keyboard }) {
    const chunks = this.#prepare(text)
    if (chunks.length === 0) return
    await this.#sendChunks({ key, kind, peerId, chunks, usePassive: true, keyboard })
  }
   ...
   */
  async #sendChunks({ key, kind, peerId, chunks, usePassive, keyboard }) {
    const { intervalMs } = this.#read()
    ...
    for (let index = 0; index < chunks.length; index += 1) {
      ...
      const body = this.#body(chunk, reply, index === chunks.length - 1 ? keyboard : undefined)
