...
export class Outbound {
  ...
  sendActive({ key, kind, peerId, text, keyboard }) {
    ...
    const next = previous
      .catch(() => {})
      .then(() => this.#sendChunks({ key, kind, peerId, chunks: this.#prepare(text, keyboard), usePassive: false, keyboard }))
    this.#chains.set(key, next)
    next.finally(() => {
      if (this.#chains.get(key) === next) this.#chains.delete(key)
    }).catch(() => {})
  ...
  sendQuoted({ key, kind, peerId, text, msgId, msgSeq }) {
    ...
    const next = previous
      .catch(() => {})
      .then(async () => {
        const chunks = this.#prepare(text, keyboard)
   ...
   */
  #prepare(text, keyboard) {
    const { maxBytes, markdownMode } = this.#read()
    const body = this.#asMarkdown(text, keyboard, markdownMode) ? text : markdownToPlain(text)
  ...
  #body(chunk, reply, keyboard) {
    ...
    const content = this.#asMarkdown(chunk, keyboard, markdownMode)
      ? { markdown: chunk }
      : { text: chunk }
...
export function canRenderAsMarkdown(text) {
  ...
  // A table needs a delimiter row. Pipes alone are not enough, because an
  // ordinary sentence may contain them.
  return !/^\s*\|?[\s:|-]*-[\s:|-]*-[\s:|-]*\|?\s*$/mu.test(source)
}
