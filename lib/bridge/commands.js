 ...
 */
export function createSessionCommands({ ctx, sessions, pending, settingsScope, restart, busy, usage, screenshot, log }) {
  ...
  return async function run({ name, args, message, key, settings, signal, reply }) {
    ...
    if (name === 'usage') {
      await runUsage({ key, sessions, usage, reply, log })
      return true
    }
...
/**
 * `/usage` — what this conversation has cost, in turns, steps and tokens.
 *
 * Read through the same `sessionStats` projection the desktop chat renders, so
 * the two surfaces cannot disagree. Listing is not administration, so any
 * admitted sender may read it — same rule as `/status`.
 *
 * @param options - command context.
 */
async function runUsage({ key, sessions, usage, reply, log }) {
  const record = sessions.get(key)
  if (record === undefined) {
    await reply('⚠️ 当前还没有 DSH 会话。先发一条消息建立会话，再用 /usage。')
    return
  }
  ...
  let stats
  try {
    stats = await usage(record.sessionId)
  } catch (error) {
    log(`${key}: usage read failed: ${String(error?.message ?? error)}`)
    await reply(`⚠️ 读取用量失败：${String(error?.message ?? error)}`)
    return
  }
  if (stats === null || stats === undefined) {
    await reply('⚠️ 读不到本会话的用量（会话可能尚未产生已完成步骤）。')
    return
  }
  await reply(formatUsage(stats))
}

/**
 * Render one session's usage figures.
 *
 * Wall times are reported the way an operator thinks about them ("4 分 12 秒"),
 * and long waits are named rather than hidden: the tool figure is usually the
 * slow part of an agentic turn, which is exactly what someone on a phone is
...
export function formatUsage(stats) {
  const lines = [
    '📈 本会话用量',
    `轮次：${String(stats.turns ?? 0)} · 步骤：${String(stats.steps ?? 0)}`,
  ]
  const llm = formatElapsed(stats.llmMs, { zero: '' })
  const tool = formatElapsed(stats.toolMs, { zero: '' })
  if (llm !== '' || tool !== '') lines.push(`模型耗时：${llm === '' ? '—' : llm} · 工具耗时：${tool === '' ? '—' : tool}`)
  const tokens = Number.isSafeInteger(stats.decodeTokens) ? stats.decodeTokens : 0
  const decode = formatElapsed(stats.decodeMs, { zero: '' })
  if (tokens > 0 || decode !== '') {
    lines.push(`输出 tokens：${tokens.toLocaleString('en-US')}${decode === '' ? '' : ` · 解码 ${decode}`}`)
  }
  if (Number.isSafeInteger(stats.ttftSteps) && stats.ttftSteps > 0) {
    const average = formatElapsed(Math.round((stats.ttftMs ?? 0) / stats.ttftSteps), { zero: '' })
    lines.push(`首 token 平均：${average === '' ? '—' : average}（${String(stats.ttftSteps)} 步）`)
  }
  lines.push('（DSH 只记首次完成步骤后的整段日志，压缩与分页不影响这些数字）')
