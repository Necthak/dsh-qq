 ...
 */
export function createSessionCommands({ ctx, sessions, pending, settingsScope, restart, busy, credit, screenshot, readLog, doctor, todos, installMenu, log }) {
  ...
  return async function run({ name, args, message, key, settings, signal, reply }) {
    ...
    if (name === 'menu') {
      await runMenu({ args, message, key, settings, installMenu, reply, log })
      return true
    }
...
/**
 * `/menu install` — put the platform's instruction panel in the chat window.
 *
 * The panel is the one surface that exists in group chats as well as direct
 * ones, and it sits there permanently, so nothing has to be remembered. A tap
 * fills the input box with the command rather than sending it, which is why the
 * items are written as the commands themselves.
 *
 * Owner-only: it writes to the bot's own configuration, not to this conversation.
 *
 * @param options - command context.
 */
async function runMenu({ args, message, key, settings, installMenu, reply, log }) {
  const action = String(args[0] ?? '').trim().toLowerCase()
  if (action !== 'install') {
    await reply('用法：/menu install —— 在聊天窗口底部安装指令面板（仅 owner）。点面板项会把命令填进输入框。')
    return
  }
  if (!isOwner(message, settings)) {
    await reply('⛔ 只有 owner 可以安装指令面板。')
    return
  }
  if (typeof installMenu !== 'function') {
    await reply('⚠️ 指令面板未接线。')
    return
  }
  let results
  try {
    results = await installMenu()
  } catch (error) {
    log(`${key}: panel install failed: ${String(error?.message ?? error)}`)
    await reply(`⚠️ 安装指令面板失败：${String(error?.message ?? error)}`)
    return
  }
  await reply(formatPanelInstall(results))
}

/**
 * Render the install result for QQ.
 *
 * @param results - one `{ scope, action, panelId, reason }` per target.
 * @returns The message body.
 */
export function formatPanelInstall(results) {
  const list = Array.isArray(results) ? results : []
  if (list.length === 0) return '⚠️ 没有可安装的目标会话。'
  const labels = { created: '已创建', updated: '已更新', failed: '失败', skipped: '跳过' }
  const scopes = { c2c: '单聊', group: '群聊' }
  const lines = ['📋 指令面板']
  for (const item of list) {
    const where = scopes[item?.scope] ?? String(item?.scope ?? '?')
    const what = labels[item?.action] ?? String(item?.action ?? '?')
    const why = typeof item?.reason === 'string' && item.reason !== '' ? `（${item.reason}）` : ''
    lines.push(`${what}：${where}${why}`)
  }
  lines.push('在聊天窗口底部点一下即可，命令会填入输入框，再按发送。')
